...
Pass 2  — per tile: load _nodes.parquet -> assign pass IDs ->
    ...
    Hash   : short SHA-256 of all statistical + grid parameters that
             ...
             GRID_RES, EMPIRICAL_SIGMA_SLOPE).
             Changing any of these produces a new hash and triggers
             regeneration; _nodes.parquet is always reused.
             NOTE: SEAS_AS_PCT_MEDIAN and TREND_AS_PCT_MEDIAN are
             intentionally excluded from the hash — they are display-only
...
Pass 3  — global: concatenate all <tile>_cell_stats_<hash>.parquet
    files (one row per cell, tiny) and plot trend and seasonal maps.
    No obs data is loaded; all heavy statistics were done in Pass 2.
